...
const ShowPatientsBills = () => {
    ...
    const loadData = async() => {
        const response = await axios.get("http://localhost:5002/api/get/patientsbillingstatement");
                ...
                {data.map((item, index) =>{
                        return (
                            <tr className='bg-white border-[1px] border-none hover:bg-gray-50 dark:hover:bg-gray-100'
                                ...
                                <td>
                                    <Link to={`/printdischargepatientdetails/${item.patient_number}`} >
                                        <button className='font-bold border-[1px] border-red-500 px-3 py-1 m-2 rounded-md hover:bg-red-400 hover:text-white '>Generate Billing Statement</button>
                                    </Link>
...
export default ShowPatientsBills;